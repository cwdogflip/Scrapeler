...
def __route_through_subpage(directory_page, subpage_id, image_file_path):
    ret = 0
    with requests.Session() as sess:
    ...
    if response.status_code == 200:
        ...
        try:
            ...
            image_file_path += extension

            delay = 3 + random.uniform(2, 3)
            time.sleep(delay)
            ret = __save_image(subpage_id, current_img, image_file_path)
        except Exception as e:
            pass
    return ret


def __save_image(referencing_page, current_img, save_to):
    with requests.Session() as sess:
        response = sess.get(current_img, data=None, stream=True,
                            headers={'User-Agent': UserAgent().firefox, 'Referer': referencing_page,})
...
def scrape_booru(waifu_name, scrape_save_directory):
    ...
    while total_saved_imgs < 52 and more_pages:
        ...
        scrape_soup = __get_soup(scrape_url)
        ...
        for num, result in enumerate(results):
            ...
            image_file_path = "{directory}\\{fn}".format(
                ...
                fn='{}.'.format(total_saved_imgs + 1)
            )
            time.sleep(2 + random.uniform(0, 2))
            total_saved_imgs += __route_through_subpage(scrape_url, referer_base.format(refer_id), image_file_path)
